...
public class MyDrawerLayout extends ViewGroup {
    ...
    public static final int NO_GRAVITY = Gravity.NO_GRAVITY;
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        for (int i = 0; i < getChildCount(); i++) {
            ...
            if (isContentView(childAt)) {
                childAt.layout(lp.leftMargin, lp.topMargin, lp.leftMargin + childAt.getMeasuredWidth(), lp.topMargin + childAt.getMeasuredHeight());
            } else if (isDrawChild(childAt)) {
    ...
    public class DrawerLayoutParams extends MarginLayoutParams {
        ...
        public DrawerLayoutParams(Context c, AttributeSet attrs) {
            super(c, attrs);
            TypedArray typedArray = c.obtainStyledAttributes(attrs, R.styleable.MyDrawerLayout);
            gravity = typedArray.getInt(R.styleable.MyDrawerLayout_android_layout_gravity, Gravity.NO_GRAVITY);
            Log.d(TAG, "DrawerLayoutParams 1: " + gravity);
            typedArray.recycle();
        }
        ...
        public DrawerLayoutParams(LayoutParams layoutParams) {
            super(layoutParams);
        }
    ...
    public void closeDrawer(View drawerView, boolean anim) {
        ...
        leftCallback.removeCallbakcs();
        rightCallback.removeCallbakcs();
        invalidate();
    }
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        boolean interceptForDrag = leftHelper.shouldInterceptTouchEvent(ev)
                | rightHelper.shouldInterceptTouchEvent(ev);
        boolean interceptForTab = false;
        isClickEvent = false;
        switch (ev.getAction()) {
            case MotionEvent.ACTION_DOWN:
                View leftView = findDrawerWithGravity(Gravity.LEFT);
                View rightView = findDrawerWithGravity(Gravity.RIGHT);
                if (leftView.getLeft() == 0 || rightView.getRight() == getMeasuredWidth()) {
                    View topChildUnder = leftHelper.findTopChildUnder((int) ev.getX(), (int) ev.getY());
                    if (topChildUnder != null && isContentView(topChildUnder)) {
                        interceptForTab = true;
                        isClickEvent = interceptForTab;
                    }
                }
                break;
        }
        return interceptForDrag || interceptForTab;
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_UP:
                View topChildUnder = leftHelper.findTopChildUnder((int) event.getX(), (int) event.getY());
                if (topChildUnder != null && isContentView(topChildUnder)) {
                    View leftVew = findDrawerWithGravity(Gravity.LEFT);
                    View rightView = findDrawerWithGravity(Gravity.RIGHT);
                    final int peekDistance = leftHelper.getEdgeSize();
                    Log.d(TAG, "onTouchEvent: " + leftVew.getLeft() + " " + rightView.getRight());
                    //left
                    if (leftVew.getLeft() == 0 && leftHelper.getViewDragState() == ViewDragHelper.STATE_IDLE && isClickEvent) {
                        closeDrawer(leftVew);
                    }
                    if (leftVew.getRight() == peekDistance) {
                        closeDrawer(leftVew);
                    }
                    // right
                    if (rightView.getRight() == getMeasuredWidth() && rightHelper.getViewDragState() == ViewDragHelper.STATE_IDLE && isClickEvent) {
                        closeDrawer(rightView);
                    }
                    if (rightView.getLeft() == getMeasuredWidth() - peekDistance) {
                        closeDrawer(rightView);
                    }
                }
                break;
